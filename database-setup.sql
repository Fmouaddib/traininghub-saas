...
('École Internationale de Paris', '123 Rue de la Paix, Paris', 'contact@eip.edu'),
('Lycée Victor Hugo', '456 Avenue des Champs, Lyon', 'admin@vh-lyon.edu');

INSERT INTO buildings (school_id, name, floors) VALUES
((SELECT id FROM schools WHERE name = 'École Internationale de Paris'), 'Bâtiment Principal', 3),
((SELECT id FROM schools WHERE name = 'École Internationale de Paris'), 'Bâtiment Sciences', 2);

INSERT INTO rooms (building_id, name, room_number, capacity, room_type, equipment) VALUES
((SELECT id FROM buildings WHERE name = 'Bâtiment Principal'), 'Salle A101', 'A101', 30, 'classroom', '["tableau", "projecteur", "ordinateur"]'),
((SELECT id FROM buildings WHERE name = 'Bâtiment Principal'), 'Salle A102', 'A102', 25, 'classroom', '["tableau", "projecteur"]'),
((SELECT id FROM buildings WHERE name = 'Bâtiment Sciences'), 'Laboratoire B201', 'B201', 20, 'laboratory', '["microscopes", "paillasse", "hotte"]'),
((SELECT id FROM buildings WHERE name = 'Bâtiment Principal'), 'Amphithéâtre', 'AMPHI', 100, 'amphitheater', '["micro", "projecteur", "écran géant"]');
...
-- Success message
SELECT 'Base de données EduPlanner créée avec succès! 🎉' AS message;