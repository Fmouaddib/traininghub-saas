-- Script de nettoyage et création pour TrainingHub
-- Exécuter ce script dans Supabase SQL Editor

-- 1. NETTOYAGE - Supprimer les anciennes tables et types
...
-- 2. CRÉATION - Nouveau schéma TrainingHub
-- Create custom types
...
CREATE INDEX idx_sessions_time ON training_sessions(start_time, end_time);
CREATE INDEX idx_sessions_status ON training_sessions(status);
CREATE INDEX idx_participants_session ON session_participants(session_id);
CREATE INDEX idx_participants_email ON session_participants(participant_email);
...
-- Success message
SELECT 'Database TrainingHub completement recree avec succes! 🎉' AS message;